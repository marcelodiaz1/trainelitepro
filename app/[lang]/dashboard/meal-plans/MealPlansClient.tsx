...
export default function MealPlansClient({ dict, lang }: { dict: any; lang: string }) {
  const t = dict?.mealPlans || {};
    ...
    <main className="bg-[#0b0b0b] text-white min-h-screen flex">
      <div className="p-8 flex-1 w-full">  
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
          <div>
            <h1 className="text-4xl font-extrabold tracking-tight text-white mb-2 uppercase italic flex items-center gap-3">
              <UtensilsCrossed className="text-orange-500" /> {t.title}
            </h1>
            <p className="text-slate-500 text-sm">
              {isAtLimit && role === "trainer" ? t.limitReached : t.subtitle}
            </p>
          </div>
        ...
        {loading ? (
          ...
          <>
            <div className="bg-[#111] border border-slate-800 rounded-2xl shadow-2xl overflow-hidden">
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-[#161616]/50 border-b border-slate-800">
                    ...
                    <SortableHeader onClick={() => handleSort("trainee_id")} label={t.table?.trainee} active={sortColumn === "trainee_id"} />
                    <SortableHeader onClick={() => handleSort("duration")} label={t.table?.duration} active={sortColumn === "duration"} /> 
                      ...
                      <tr key={plan.id} className="hover:bg-white/[0.02] transition-colors group">
                        ...
                        <td className="px-6 py-5">
                          <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase border ${getDurationColor(plan.duration)}`}>
                            {t.durations?.[plan.duration] || plan.duration}
                          </span>
                        </td>
                    ...
                    <tr>
                      <td colSpan={5} className="py-20">
                        <div className="flex flex-col items-center justify-center text-center">
                          ...
                          <h3 className="text-lg font-bold text-white mb-1">{t.empty?.title}</h3>
                          <p className="text-slate-500 text-sm max-w-xs mx-auto mb-6">
                            {role === "trainer" ? t.empty?.trainer : t.empty?.trainee}
                          </p>
                        </div>
                      </td>
                    </tr>
