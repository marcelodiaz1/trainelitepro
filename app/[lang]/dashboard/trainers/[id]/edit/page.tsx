...
export default function EditTrainerPage() {
  ...
  useEffect(() => {
    const fetchTrainer = async () => {
      ...
      if (fetchError || !data) {
        setError("Trainer not found or error loading data.");
      } else {
        setFormData(data);
      }
      setLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (updateError) {
      ...
      router.push(`/dashboard/trainers/${id}`);
      router.refresh();
    }
  };
  ...
  return (
    <main className="bg-[#050505] text-slate-200 min-h-screen flex font-sans">
      ...
      <div className="p-8 flex-1 max-w-3xl mx-auto w-full">
        {/* Header */}
        <LocalizedLink href={`/dashboard/trainers/${id}`} className="flex items-center gap-2 text-slate-500 hover:text-white transition-colors mb-8 text-xs font-bold uppercase tracking-widest">
          <ChevronLeft size={16} /> Discard Changes
        </LocalizedLink>

        <div className="mb-10">
          <h1 className="text-4xl font-extrabold tracking-tight text-white mb-2 uppercase italic">Edit Profile</h1>
          <p className="text-slate-500 text-sm">Update credentials and professional status for this staff member.</p>
        </div>
