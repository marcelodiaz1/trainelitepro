...
export default function NewTrainerPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    specialty: "",
    status: "active",
    role: "trainer", // Explicitly set as trainer
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Success! Redirect back to the trainers list
    ...
    <main className="bg-[#050505] text-slate-200 min-h-screen flex font-sans">
      <div className="p-8 flex-1 max-w-3xl mx-auto w-full">
        ...
        <div className="mb-10">
          ...
          <p className="text-slate-500 text-sm">Create a new professional profile for your coaching staff.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8 bg-[#111] border border-slate-800 rounded-3xl p-8 shadow-2xl">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                required
                type="email"
                placeholder="trainer@elitepro.com"
                className={inputClasses}
          ...
          <div className="pt-4 border-t border-slate-800/50 flex flex-col sm:flex-row gap-4 items-center justify-between">
            <p className="text-[10px] text-slate-500 font-medium uppercase tracking-widest">
              Role: <span className="text-blue-500 font-black">Trainer</span>
            </p>
            <button
              type="submit"
              disabled={loading}
        ...
        <div className="mt-8 bg-blue-500/5 border border-blue-500/10 rounded-2xl p-6 flex gap-4">
          ...
          <div>
            <h4 className="text-sm font-bold text-white mb-1 tracking-tight">Staff Onboarding</h4>
            <p className="text-xs text-slate-400 leading-relaxed">
              This trainer will receive an automated confirmation email. Their account is set up with full access to the staff directory by default.
            </p>
          </div>
        </div>
      </div>
