...
export default function TrainerProfilePage({ dict }: { dict: any }) {
  const t = dict?.trainerProfile;
  const { id } = useParams();
      ...
      <section className="max-w-6xl mx-auto py-32 px-6">
        ...
        <div className="flex flex-col md:flex-row gap-12 mb-20 items-center md:items-start">
          ...
          <div className="flex-1 text-center md:text-left">
            ...
            <p className="text-[#ff6b1a] text-xl font-black uppercase tracking-[0.3em] mb-6 italic">
              {trainer.specialty || t.defaultSpecialty}
            </p>
            ...
            <div className="flex flex-wrap justify-center md:justify-start gap-4">
              ...
              >
                <CreditCard size={20} /> {t.getStarted}
              </button>
              <a 
        ...
        <div className="mb-20">
          <div className="flex items-center gap-4 mb-10">
            <h2 className="text-2xl font-black italic uppercase tracking-tighter">{t.chooseTier}</h2>
                ...
                >
                  <div>
                    <div className="flex justify-between items-start mb-6">
                      ...
                      <div className="bg-[#ff6b1a] text-black text-[10px] font-black px-3 py-1 rounded-full uppercase italic">{t.hot}</div>
                    </div>
                  ...
                  >
                    {t.selectPlan} <ChevronRight size={18} />
                  </button>
                </motion.div>
              ))
            ) : (
              <div className="col-span-3 py-20 text-center border-2 border-dashed border-white/5 rounded-[3rem]">
                <p className="text-gray-600 font-black uppercase italic tracking-widest">{t.noPlans}</p>
              </div>
            )}
          </div>
        </div>
            ...
            >
              <div className="p-10">
                <div className="flex justify-between items-start mb-10">
                  <div>
                    <h2 className="text-3xl font-black italic uppercase tracking-tighter text-white">{t.payment.title}</h2>
                ...
                <div className="space-y-6 bg-white/[0.02] p-8 rounded-[2rem] border border-white/5 mb-10">
                  <ModalDetail label={t.payment.beneficiary} value={trainer.bank_full_name} dict={dict} />
                  <ModalDetail label={t.payment.bank} value={trainer.bank_name} dict={dict} />
                  <ModalDetail label={t.payment.accountNumber} value={trainer.bank_account_number} dict={dict} />
                  <ModalDetail label={t.payment.idNumber} value={trainer.bank_rut} dict={dict} />
                  <ModalDetail label={t.payment.accountType} value={trainer.bank_account_type} dict={dict} />
                </div>

                <CopyButton trainer={trainer} dict={dict} />
...
function CopyButton({ trainer, dict }: { trainer: any; dict: any }) {
  const [copied, setCopied] = useState(false);
  const pt = dict.trainerProfile.payment;

  const handleCopy = () => {
    const text = `
${pt.beneficiary}: ${trainer?.bank_full_name || '-'}
${pt.bank}: ${trainer?.bank_name || '-'}
${pt.accountNumber}: ${trainer?.bank_account_number || '-'}
${pt.idNumber}: ${trainer?.bank_rut || '-'}
${pt.accountType}: ${trainer?.bank_account_type || '-'}
    `.trim();
  ...
  return (
    ...
    >
      ...
      {copied ? pt.copied : pt.copyInfo}
    </button>
  );
}

function ModalDetail({ label, value, dict }: { label: string; value?: string; dict: any }) {
  return (
    <div>
      <p className="text-[10px] font-black text-gray-600 uppercase tracking-[0.2em] mb-1">{label}</p>
      <p className="text-md font-bold text-white tracking-tight italic">{value || dict.trainerProfile.payment.unset}</p>
    </div>
  );
}