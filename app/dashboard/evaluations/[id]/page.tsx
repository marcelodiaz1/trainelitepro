    ...
    <main className="bg-[#050505] text-white min-h-screen flex">
      ...
      {/* Header */}
        <div className="flex justify-between items-end mb-12">
          <div>
            <div className="flex items-center gap-3 mb-2">
               <span className="bg-orange-500 text-[10px] font-black px-2 py-0.5 rounded text-black uppercase">Final Report</span>
               <span className="text-slate-500 font-mono text-xs">{new Date(data.created_at).toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}</span>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          {/* LEFT: Core Composition */}
          <div className="lg:col-span-3 space-y-4">
            <h3 className="text-xs font-black uppercase tracking-[0.3em] text-slate-700 mb-6 flex items-center gap-2">
              <Dna size={14} /> Core Metrics
            </h3>
            <MetricCard label="Weight" value={data.weight} unit="kg" icon={Scale} />
            <MetricCard label="Fat Percentage" value={data.fat_percentage} unit="%" icon={Activity} />
            <MetricCard label="Lean Mass" value={data.lean_mass} unit="kg" icon={Target} color="blue" />
            <MetricCard label="Fat Mass" value={data.fat_mass} unit="kg" icon={Move} color="red" />
          </div>

          {/* CENTER: The Body Visualizer */}
          <div className="lg:col-span-6 relative bg-[#0a0a0a] border border-slate-900 rounded-[4rem] flex items-center justify-center py-12">
            <div className="absolute inset-0 opacity-20 pointer-events-none" 
                 style={{ backgroundImage: 'radial-gradient(#333 1px, transparent 1px)', backgroundSize: '30px 30px' }} />
            
            {/* Anatomical Map Lines & Data */}
            <div className="relative w-full h-full max-w-md">
              
              {/* Image Placeholder Tag for Human Anatomy UI */}
              <div className="flex justify-center">
                 <img 
                    src="/body.jpg" 
                    alt="Anatomy Diagram"style={{ filter: "invert(1)" }}
                    className="h-[500px] w-auto object-contain mix-blend-screen opacity-80 "
                    />
              </div>
a
              {/* Data Callouts - Positioned around the body */}
              <BodyCallout side="left" top="15%" label="Shoulder Blade" value={data.shoulder_blade} />
              <BodyCallout side="right" top="20%" label="Arm / Biceps" value={`${data.arm} / ${data.biceps}`} />
              <BodyCallout side="left" top="35%" label="Torso / Chest" value={data.torso} />
              <BodyCallout side="right" top="45%" label="Waist" value={data.waist} />
              <BodyCallout side="left" top="55%" label="Suprailiac" value={data.suprailiac} />
              <BodyCallout side="right" top="65%" label="Hip" value={data.hip} />
              <BodyCallout side="left" top="75%" label="Leg / Thigh" value={data.leg} />
              <BodyCallout side="right" top="85%" label="Shin / Wrist" value={`${data.shin} / ${data.wrist}`} />

              {/* Central Scanner Effect */}
              <motion.div 
                animate={{ top: ['0%', '100%', '0%'] }}
          ...
          {/* RIGHT: Detailed Fold Metrics */}
          <div className="lg:col-span-3">
             <h3 className="text-xs font-black uppercase tracking-[0.3em] text-slate-700 mb-6 flex items-center gap-2">
              <Activity size={14} /> Fold Analysis
            </h3>
            <div className="space-y-2 bg-[#111] p-6 rounded-3xl border border-slate-800">
                <FoldItem label="Triceps" value={data.triceps} />
                <FoldItem label="Biceps" value={data.biceps} />
                <FoldItem label="Subscapular" value={data.shoulder_blade} />
                <FoldItem label="Suprailiac" value={data.suprailiac} />
                <FoldItem label="Torso" value={data.torso} />
                <FoldItem label="Addition" value={data.addition} highlight />
                
                <div className="mt-8 pt-6 border-t border-slate-800">
                   <p className="text-[10px] font-black text-slate-600 uppercase mb-4 tracking-widest">Calculated IMC</p>
