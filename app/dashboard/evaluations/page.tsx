      ...
      <main className="bg-[#0b0b0b] text-white min-h-screen flex">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center h-96 text-slate-500">
            ...
            <p className="animate-pulse font-medium tracking-widest text-[10px]">SYNCING METRICS</p>
          </div>
        ) : evals.length === 0 ? (
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-[#161616]/50 border-b border-slate-800 text-white">
                    <SortableHeader onClick={() => handleSort("created_at")} label="Date" active={sortColumn === "created_at"} />
                    <SortableHeader onClick={() => handleSort("trainee_id")} label="Athlete" active={sortColumn === "trainee_id"} />
                    <SortableHeader onClick={() => handleSort("weight")} label="Weight" active={sortColumn === "weight"} icon={<Scale size={12}/>} />
                    <SortableHeader onClick={() => handleSort("fat_percentage")} label="Fat %" active={sortColumn === "fat_percentage"} />
                    <th className="px-6 py-4 text-[10px] font-black uppercase tracking-[0.2em] text-slate-600">Result</th>
                  ...
                  {evals.map((e) => (
                    <tr key={e.id} className="hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="px-6 py-4 text-right relative">
                        <button
                          onClick={() => setDropdownOpen(dropdownOpen === e.id ? null : e.id)}
                          ...
                          {dropdownOpen === e.id && (
                            <>
                              ...
                              <motion.div
                                ...
                                className="absolute right-6 mt-2 w-48 bg-[#1a1a1a] border border-slate-700 rounded-xl shadow-2xl z-20 overflow-hidden text-left"
                              >
                                <Link href={`/dashboard/evaluations/${e.id}`}>
                                ...
                                <Link href={`/dashboard/evaluations/${e.id}/edit`}>
                                ...
                                <button
                                  ...
                                  className="flex items-center gap-3 w-full px-4 py-3 text-[11px] font-bold uppercase tracking-tight hover:bg-red-500/10 text-red-400 border-t border-slate-800 transition-colors"
                                >
