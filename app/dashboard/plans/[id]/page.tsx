        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-12">
          <div>
            ...
            <h1 className="text-5xl font-extrabold italic uppercase tracking-tighter">
              {plan.title} <span className="text-[#ff6b1a]">Details</span>
            </h1>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            <section className="bg-[#111] border border-slate-800 rounded-3xl p-8 shadow-2xl">
              <h2 className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em] mb-6 flex items-center gap-2">
                <ShieldCheck size={14} className="text-[#ff6b1a]" /> Core configuration
              </h2>
              
              <div className="grid sm:grid-cols-2 gap-8">
                <div>
                  <p className="text-slate-500 text-[10px] font-bold uppercase mb-1">Display Price</p>
                  <p className="text-3xl font-bold text-white tracking-tight">{plan.price}</p>
                </div>
                <div>
                  <p className="text-slate-500 text-[10px] font-bold uppercase mb-1">Created On</p>
              ...
              <div className="mt-10 pt-8 border-t border-slate-800/50">
                <p className="text-slate-500 text-[10px] font-bold uppercase mb-4">Feature Set Preview</p>
          ...
          <div className="space-y-6">
            <section className="bg-[#111] border border-slate-800 rounded-3xl p-6">
              <h3 className="text-[10px] font-black uppercase text-slate-500 mb-6 flex items-center gap-2">
                <CreditCard size={14} /> Integration
              </h3>
              
              <div className="space-y-6">
                <div>
                  <label className="text-[9px] font-bold text-slate-600 uppercase block mb-2">PayPal Button ID</label>
                  <code className="bg-black px-3 py-2 rounded-lg text-[#ff6b1a] text-xs font-mono block border border-slate-800">
                    {plan.paypal_button_id || "NOT_CONFIGURED"}
                  </code>
                </div>

                <div>
                  <label className="text-[9px] font-bold text-slate-600 uppercase block mb-2">Internal CTA Path</label>
                  {plan.cta ? (
                    <Link href={plan.cta} className="text-blue-400 text-xs flex items-center gap-1 hover:underline">
                      {plan.cta} <ExternalLink size={12} />
                    </Link>
                  ) : (
                    <span className="text-slate-700 text-xs italic">No manual CTA set</span>
                  )}
                </div>

                <div>
                  <label className="text-[9px] font-bold text-slate-600 uppercase block mb-2">Database ID</label>
            ...
            <div className="bg-gradient-to-br from-[#ff6b1a]/10 to-transparent border border-[#ff6b1a]/20 rounded-3xl p-6">
              <p className="text-[10px] font-bold text-[#ff6b1a] uppercase mb-2">Public Preview</p>
              <p className="text-xs text-slate-400 leading-relaxed">
                This is how the plan appears to potential customers on your landing page. Ensure the price string matches your PayPal Hosted Button configuration to avoid billing discrepancies.
              </p>
            </div>
          </div>
        </div>
