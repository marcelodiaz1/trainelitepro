...
interface Exercise {
  id: number;
  title: string;
  field_exercise_type: string; // This stores the ID from exercise_type table
  field_zone: string;
...
export default function ExercisesTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      // 1. Fetch Zones and Exercise Types (Corrected table name)
      const [zoneRes, typeRes] = await Promise.all([
        supabase.from("zone").select("*"),
        supabase.from("exercise_type").select("*") // Corrected table name
      ]);
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
          <div>
            <h1 className="text-4xl font-extrabold tracking-tight text-white mb-2 uppercase italic flex items-center gap-3">
              <Activity className="text-orange-500" />Exercises</h1>
            <p className="text-slate-500 text-sm">Database management for workout movements.</p>
          </div>
          ...
          <div className="flex gap-3 w-full md:w-auto">
            <div className="relative flex-1 md:w-64">
              ...
              <input 
                ...
                className="w-full bg-[#111] border border-slate-800 rounded-lg py-2.5 pl-10 pr-4 text-xs focus:border-blue-500 outline-none transition-all placeholder:text-slate-700 text-white"
              />
            </div>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-[#161616]/50 border-b border-slate-800">
                    <SortableHeader onClick={() => handleSort("title")} label="Exercise Name" active={sortColumn === "title"} />
                    <SortableHeader onClick={() => handleSort("field_zone")} label="Zone" active={sortColumn === "field_zone"} />
                    <SortableHeader onClick={() => handleSort("field_exercise_type")} label="Type" active={sortColumn === "field_exercise_type"} />
                    <th className="px-6 py-4 text-[10px] uppercase tracking-[0.2em] text-slate-600 text-right">Preview</th>
                    ...
                    <tr key={exercise.id} className="hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="px-6 py-4 text-sm">
                        <span className="px-2 py-1 rounded bg-orange-500/10 border border-orange-500/20 text-[10px] font-black uppercase text-orange-400"> 
                          {/* Lookup from the exercise_type array using the ID */}
                      ...
                      <td className="px-6 py-4 text-right relative">
                        ...
                        {openDropdownId === exercise.id && (
                          <>
                            ...
                            <div className="absolute right-6 mt-2 w-40 bg-[#1a1a1a] border border-slate-700 rounded-xl shadow-2xl z-20 overflow-hidden animate-in fade-in zoom-in duration-100">
                              <Link 
                                href={`/dashboard/exercises/${exercise.id}`}
                                className="w-full flex items-center gap-3 px-4 py-3 text-xs font-medium hover:bg-slate-800 transition-colors text-slate-300 border-b border-slate-800"
                              >
                                <ExternalLink size={14} className="text-orange-500" /> View Details
                              </Link>
                              ...
                              <button 
                                ...
                                className="w-full flex items-center gap-3 px-4 py-3 text-xs font-medium hover:bg-red-500/10 transition-colors text-red-400 border-t border-slate-800"
                              >
